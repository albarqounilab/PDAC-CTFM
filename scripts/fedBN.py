...
save_dir      = os.environ.get("SAVE_DIR",      os.path.join(os.path.dirname(__file__), "..", "results", "fedbn"))
...
def get_bn_keys(model):
    """Dynamically get state_dict keys for all BatchNorm layers in the model."""
    bn_keys = []
    for name, module in model.named_modules():
        if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d)):
            for key in module.state_dict().keys():
                bn_keys.append(f"{name}.{key}")
    return set(bn_keys)
...
def FedUpdate(global_model, local_models):
    ...
    for local_model in local_models:
        ...
        for key in global_dict.keys():
            # Exclude strictly Batch Normalization layers from being overwritten
            if key not in bn_keys:
                local_dict[key].data.copy_(global_dict[key].data)

def FedBN(global_model, local_models, client_weights):
    new_model_dict = None
    bn_keys = get_bn_keys(global_model)
    
    for i in range(len(local_models)):
        local_dict = local_models[i].state_dict()
        # Exclude ALL strictly BN stats and params (including affine weight/bias)
    ...
    # Load aggregated parameters into global_model (strict=False because we omitted norm keys)
...
def main_fl(pretrained_path=None,learning_rate=0.001, weight_decay=0.01, num_rounds= 34,local_epochs = 3 ):
    ...
    run_name = (
        f"3011_flbn_bs{batch_size}_rounds{num_rounds}_localepochs_{local_epochs}_"
    ...
    for round_num in range(num_rounds):
        ...
        FedBN(global_model, local_models, client_weights)
        ...
        # Per-center already in evaluate_fl wandb.log
        ...
        # Save additionally if balanced accuracy crosses a useful threshold
...
if __name__ == "__main__":
    num_rounds = 20  # same for all
    combinations = [
        (1e-3, 1e-2, 3),   # lr=1e-3, wd=1e-5, local_epochs=3
        (1e-3, 1e-5, 3),   # lr=1e-3, wd=1e-5, local_epochs=3
        (1e-4, 1e-5, 3),   # lr=1e-4, wd=1e-5, local_epochs=3
        (1e-3, 1e-4, 2),   # lr=2e-5, wd=1e-4, local_epochs=2
        (1e-3, 1e-4, 3),   # lr=2e-5, wd=1e-4, local_epochs=2
        (1e-5, 5e-4, 2),   # lr=1e-5, wd=5e-4, local_epochs=2
        (1e-5, 5e-4, 3),   # lr=1e-5, wd=5e-4, local_epochs=3
        (2e-5, 1e-4, 2),   # lr=2e-5, wd=1e-4, local_epochs=2
        (2e-5, 1e-4, 3),   # lr=2e-5, wd=1e-4, local_epochs=3
        
    ]

    for lr, wd, local_epoch in combinations:
        # print(f"Running: lr={lr}, wd={wd}, local_epochs={local_epoch}, num_rounds={num_rounds}")
